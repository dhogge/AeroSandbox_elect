...
electric motors. Because it is on the centerline, it produces zero
yaw moment, reducing V_mc and allowing a smaller vertical tail.
...
# Tail geometry
tail_arm = 6.0                 # m, wing AC to tail AC
...
design_mass_TOGW = opti.variable(
    init_guess=7700, log_transform=True, lower_bound=4000, upper_bound=12500
)
...
# Battery longitudinal placement (optimizer chooses for best CG balance)
# Bounded within the cabin floor: from just aft of cockpit to aft cabin bulkhead
x_cg_battery = opti.variable(
    init_guess=nose_length + 0.25 * cabin_length,
    lower_bound=nose_length + 0.5,                           # 0.5 m aft of cockpit bulkhead
    upper_bound=nose_length + cabin_length - 0.5,            # 0.5 m fwd of aft cabin bulkhead
)  # meters from nose
...
# --- OEI Climb Gradient (FAR 23, 2 wingtip engines + BLI centerline) ---
...
# best split (`thrust_wingtip_oei_reduced`) that satisfies BOTH the
# climb-gradient AND V_mc constraints simultaneously.
...
# for component CG locations (including aft BLI hardware mass).
# SM aft >= 5%: minimum static stability at zero-fuel weight (aft CG)
# SM fwd <= 40%: limit forward CG to ensure elevator trim authority
opti.subject_to(static_margin >= 0.05)           # >=5% MAC at aft CG (stability)
opti.subject_to(static_margin_TOGW <= 0.20)      # <=20% MAC at fwd CG (trim authority)
...
opti.subject_to(bli_thrust_at_liftoff <= thrust_at_liftoff * 0.60)  # BLI ≤ 40% of total
