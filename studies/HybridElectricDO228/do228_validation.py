"""
Validation Case: Dornier DO 228-212 -- Conventional 19-Pax Turboprop
=====================================================================

Validates the AeroSandbox weight/aero/propulsion framework by modeling
a conventional (non-hybrid) turboprop configured to Dornier DO 228-212
requirements and comparing optimizer output to known real-aircraft data.

Real Aircraft Reference (Dornier DO 228-212, passenger variant):
    MTOW:            6,400 kg (14,110 lb)
    Empty Weight:    ~3,405 kg (~7,507 lb)
    Fuel Capacity:   ~1,960 L (~1,607 kg)
    Cruise Speed:    180 KTAS at FL100
    Engines:         2x Honeywell TPE331-10, 776 shp each (~155 kg dry)
    Propellers:      Hartzell 4-blade, 106 in (2.69 m)
    Wing Span:       16.97 m (55 ft 8 in)
    Wing Area:       32.0 m² (344 ft²)
    AR:              9.0 (cantilever high wing, TNT airfoil)
    TO Distance:     ~620 m (~2,035 ft) over 50 ft obstacle
    LDG Distance:    ~530 m (~1,740 ft)
    Range (max pld): ~400 nmi
    Landing Gear:    Fixed tricycle (fuselage-mounted sponsons)
    Pressurization:  Unpressurized
    Tail:            Conventional (low-mounted horizontal stabilizer)

Notes on the DO 228 wing:
    The DO 228 uses Dornier's TNT (Tragflügel neuer Technologie / New
    Technology Wing) design featuring a rectangular constant-chord center
    section and tapered outer panels.  This planform is modeled with a
    kink at ~35 % semi-span where the taper begins.
"""
...
n_pax = 19
n_crew = 2
payload_mass = 5100 * u.lbm              # ~2313 kg (close to DO 228 max payload of 2329 kg)
cruise_speed = 180 * u.knot              # 92.6 m/s
cruise_altitude = 10000 * u.foot         # 3048 m (FL100, unpressurized)
field_length_req = 2100 * u.foot         # 640 m (DO 228 STOL capability)
n_engines = 2
...
# Fuselage geometry (DO 228-212)
fuse_length = 16.56            # m (54.3 ft)
fuse_cabin_width = 1.74        # m (~rectangular cross-section, 1+2 abreast)
fuse_cabin_height = 1.70       # m
nose_length = 2.5              # m
cabin_length = 7.5             # m (19 pax, 1+2 abreast, ~0.76 m pitch)
tail_length = fuse_length - nose_length - cabin_length  # ~6.56 m

# Tail geometry (conventional low-mounted hstab)
tail_arm = 8.0                 # m, wing AC to tail AC
hstab_span_val = 5.5           # m
hstab_root_chord_val = 1.4     # m
hstab_tip_chord_val = 0.70     # m
vstab_span_val = 2.3           # m
...
# Modern composite propeller weight correction.
# The Torenbeek 1976 model was calibrated against heavy aluminum/steel
# propellers of that era.  Modern composite propellers (Hartzell, MT-Propeller)
# are roughly 3x lighter.  For reference:
#   Hartzell HC-B4TN-5 (4-blade, 2.69 m, 776 shp): ~42 kg
#   Torenbeek model prediction for same:             ~200 kg
# A factor of 0.35 brings the model in line with modern composite propellers.
modern_propeller_weight_factor = 0.35
...
design_mass_TOGW = opti.variable(
    init_guess=6400, log_transform=True, lower_bound=4000, upper_bound=9000
)
wing_span = opti.variable(
    init_guess=17.0, lower_bound=12, upper_bound=22
)
wing_root_chord = opti.variable(
    init_guess=2.25, lower_bound=1.5, upper_bound=3.5
)
cruise_alpha = opti.variable(
    init_guess=3.0, lower_bound=-2, upper_bound=10
)
mass_turboshaft_per_engine = opti.variable(
    init_guess=155, log_transform=True, lower_bound=50, upper_bound=300
)
propeller_diameter = opti.variable(
    init_guess=2.69, lower_bound=2.0, upper_bound=3.0
)
fuel_mass = opti.variable(
    init_guess=800, log_transform=True, lower_bound=100, upper_bound=2000
)
thrust_at_liftoff = opti.variable(
    init_guess=6400 * g * 0.30, log_transform=True, lower_bound=5000
)

# Derived wing geometry
# DO 228: rectangular center section, taper ratio applies to outer panels
wing_taper_ratio = 0.50
wing_tip_chord = wing_root_chord * wing_taper_ratio
wing_mean_chord = (wing_root_chord + wing_tip_chord) / 2

##### Section: Aircraft Geometry #####

# --- Fuselage (slab-sided, near-rectangular cross-section) ---
fuse = asb.Fuselage(
    name="Fuselage",
    xsecs=[
        ...
        asb.FuselageXSec(  # Cabin start
            ...
            shape=3.0,  # Nearly rectangular cross-section
        ),
        ...
        asb.FuselageXSec(  # Tail mid
            xyz_c=[nose_length + cabin_length + tail_length * 0.6, 0, 0.20],
            width=fuse_cabin_width * 0.45,
            height=fuse_cabin_height * 0.45,
            shape=2.0,
        ),
        asb.FuselageXSec(  # Tail tip
            xyz_c=[fuse_length, 0, 0.35],
            width=0.15,
            height=0.15,
        ),
    ],
...
# --- Main Wing (HIGH wing, cantilever, TNT planform) ---
# The DO 228 wing has a rectangular center section to ~35% semi-span,
# then tapers on the outer panels.
wing_x_le = 0.38 * fuse_length - 0.25 * wing_root_chord
wing_z_le = 0.5 * fuse_cabin_height  # High wing: on top of fuselage

kink_eta = 0.35  # Rectangular section ends at 35% semi-span
...
wing = asb.Wing(
    ...
    xsecs=[
        ...
        asb.WingXSec(  # Kink (end of rectangular center section)
            xyz_le=[
                0,                                      # No sweep on center section
                kink_eta * wing_span / 2,
                0,                                      # Center section is flat
            ],
            chord=wing_root_chord,                      # Same chord (rectangular)
            twist=0,
        ...
        asb.WingXSec(  # Tip
            xyz_le=[
                (1 - kink_eta) * wing_span / 2 * np.tand(5),   # LE sweep on outer panel
                wing_span / 2,
                (1 - kink_eta) * wing_span / 2 * np.tand(3),   # 3° dihedral on outer panel
            ],
...
# --- Horizontal Stabilizer (conventional, low-mounted) ---
...
hstab_z_le = 0.3  # Low-mounted on fuselage (conventional tail)
...
airplane = asb.Airplane(
    name="Dornier DO 228-212 (Validation)",
...
# DO 228 has fixed landing gear fairings, slab-sided fuselage with
# higher interference drag, antennas, scoops, door gaps, cooling inlets.
# Smaller aircraft than SkyCourier but angular fuselage shape.
CDA_misc = 0.30  # m² flat-plate equivalent drag area
...
# Wing (Torenbeek, cantilever -- no strut bracing)
m_wing = torenbeek_wt.mass_wing(
    ...
    main_gear_mounted_to_wing=False,  # Gear on fuselage sponsons
)
...
# Vertical stabilizer (Raymer) -- conventional tail (no T-tail penalty)
...
m_fuselage = raymer_wt.mass_fuselage(
    ...
    main_wing=wing,
    n_cargo_doors=1,
    landing_gear_mounted_on_fuselage=True,  # DO 228 fuselage-mounted sponsons
)
...
m_nlg = raymer_wt.mass_nose_landing_gear(
    nose_gear_length=0.5,
    design_mass_TOGW=design_mass_TOGW,
    n_wheels=2,
) * fixed_gear_weight_factor

# Nacelles (TPE331 compact nacelles, under-wing pylons)
nacelle_length = 1.6
nacelle_width = 0.65
nacelle_height = 0.65
...
m_electrical = raymer_wt.mass_electrical(
    system_electrical_power_rating=30000,  # Conventional (no hybrid bus)
...
m_ac = raymer_wt.mass_air_conditioning(
    ...
    mass_uninstalled_avionics=150 * u.lbm,  # Lighter avionics for smaller aircraft
)
...
m_propeller_each = m_propeller_each * modern_propeller_weight_factor
m_propellers_total = m_propeller_each * n_engines

# Gearbox (TPE331 gas generator → propeller shaft)
turboshaft_output_rpm = 41000
propeller_rpm = 2000
...
opti.subject_to(wing_aspect_ratio >= 8.0)   # DO 228 AR ~9.0
...
# Minimize MTOW for a conventional design -- compare to real 14,110 lb.
...
print("=" * 72)
print("   DORNIER DO 228-212 VALIDATION -- DESIGN SUMMARY")
print("=" * 72)
...
weight_items = [
    ...
    ("Propellers (composite)", sol(m_propellers_total)),
...
# =====================================================================
# VALIDATION: Compare to Real Dornier DO 228-212
# =====================================================================
print("\n" + "=" * 72)
print("   VALIDATION vs REAL DORNIER DO 228-212")
print("=" * 72)

real = {
    "MTOW (lb)":          (TOGW / u.lbm,               14110),
    "Empty Wt (lb)":      (m_empty_sol / u.lbm,        7507),
    "Wing Span (ft)":     (b / u.foot,                  55.67),
    "Wing Area (ft²)":    (S_wing / u.foot**2,          344),
    "Aspect Ratio":       (AR,                          9.0),
    "Engine SHP (each)":  (sol(power_per_turboshaft) / u.horsepower, 776),
    "Engine Mass (kg)":   (sol(mass_turboshaft_per_engine),          155),
    "Prop Dia (in)":      (sol(propeller_diameter) / 0.0254,         106),
    "Fuel (lb)":          (m_fuel_sol / u.lbm,          1468),
}
...
print("\n  Notes:")
print("  - DO 228 uses cantilever high wing (no strut bracing)")
print("  - Fixed landing gear on fuselage-mounted sponsons")
print("  - TPE331-10 has integral reduction gearbox (155 kg includes gearbox)")
print("  - Propeller weight uses 0.35x factor (modern composite vs 1976 aluminum)")
print("  - Real DO 228 fuel capacity is for full tanks (~1,037 nmi ferry)")
print("    Model sizes fuel for 400 nmi max payload range + reserves only")
